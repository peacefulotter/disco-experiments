...
def train_base(
    ...
    while itr < iterations:
        for microstep_idx in range(acc_steps):  # gradient accumulation
            x, y = get_batch(
                data["train"], sequence_length, batch_size, device=extra_args.device
            )
            with type_ctx:
        ...
        if (
            True
        ):  # itr % eval_freq == 0 or itr == iterations: # from here it's only evaluation code, all the training is above
