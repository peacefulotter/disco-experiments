...
export function categoricalCrossentropy(
    target: tf.Tensor, output: tf.Tensor, fromLogits = false): tf.Tensor {
  return tf.tidy(() => {
    if (fromLogits) {
    ...
    return tf.neg(tf.sum(
        tf.mul(target.toFloat(), tf.log(output)), output.shape.length - 1));
  });
}

export function sparseCategoricalCrossentropy(target: tf.Tensor, output: tf.Tensor): tf.Tensor {
  return tf.tidy(() => {
...
const a = tf.tensor2d([[0.1, 0.2, 0.7], [0.2, 0.3, 0.5], [0.3, 0.2, 0.5]], [3, 3]);
