...
def train_base(model, opt, data, scheduler, iterations, acc_steps, batch_size, sequence_length, eval_freq, ckpt_path, distributed_backend, extra_args):
    ...
    while itr < iterations:
        ...
        if True: # itr % eval_freq == 0 or itr == iterations: # from here it's only evaluation code, all the training is above
            if distributed_backend.is_master_process():
                ...
                val_acc, val_loss, val_perplexity = 0, 0, 0 # eval(model, data['val'], sequence_length, batch_size,
